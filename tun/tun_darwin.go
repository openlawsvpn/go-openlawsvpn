...
// macOS TUN support via NEPacketTunnelProvider.
//
// On macOS apps cannot open /dev/net/tun directly. Instead the Network Extension
// framework creates the TUN interface and exposes the raw file descriptor via
// a private KVC key on NEPacketTunnelFlow (macOS):
//
...
// On macOS these are informational only — NEPacketTunnelNetworkSettings has
...
// Name returns the interface name ("utun0" on macOS/macOS).
...
// Configure is a no-op on macOS: NEPacketTunnelNetworkSettings already configured
// the interface before handing us the fd.
func (d *Device) Configure(_ Config) error { return nil }

// OpenFd wraps a TUN file descriptor provided by macOS's NEPacketTunnelFlow (macOS).
//
// The fd is the value extracted from packetFlow via the mTunFileDescriptor KVC
// key (a private but App-Store-approved API used by WireGuard-macOS since 2019).
